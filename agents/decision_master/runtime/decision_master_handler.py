...
from decision_master import DecisionMasterAgent
...
AGENT = DecisionMasterAgent(slim=True)
...
CAPABILITY_DISPATCH = {
    "Decision-Diary.Manage":      AGENT.decision_diary_manage,
    "Decision-Events.Identify":   AGENT.decision_events_identify,
    "Decision-Outcome.Identify":  AGENT.decision_outcome_identify,
    "Decision-Outcome.Analyze":   AGENT.decision_outcome_analyze,
}
...
def lambda_handler(event, context):
    ...
    try:
        ...
        schema_url = (
            ...
            f"{capability_id.lower().replace('.', '-').replace(' ', '-')}"
            f".input.schema.json"
        )
        ...
        LOG.write(
            ...
            message="Unhandled exception in decision_master_handler.",
