# /http/agents/purchasing_agent/purchasing_agent_handler.py

"""
AI-first Lambda handler for the Purchasing-Agent.
...
Use api_handler.py (Mode 3) for:
  ...
  - Capabilities touching governed personal data (vault, budget store)

Agent lifecycle:
  One slim PurchasingAgent instance per Lambda execution environment.
...
Capabilities served: all 14 PurchasingAgent capabilities.
Routes loaded from shared/artifacts/capability_map.yaml.
"""
...
from agents.purchasing_agent.runtime.purchasing_agent import PurchasingAgent
...
def _load_pa_routes() -> dict[str, str]:
    """
    Load PurchasingAgent capability route map from capability_map.yaml.
    ...
    try:
        ...
        if map_path.exists():
            ...
            path_map = {}
            pa_routes = raw.get("routes", {}).get("purchasing-agent", [])
            for route in pa_routes:
                path = route.get("path", "")
        ...
        print(f"[pa_handler] WARNING: capability_map.yaml failed to load: {e}")

    # Hardcoded fallback — matches capability_map.yaml routes section
    return {
        "purchaseOptionsRecommend":    "Purchase-Options.Recommend",
        "purchaseRiskEvaluate":        "Purchase-Risk.Evaluate",
        "purchaseHistoryGenerate":     "Purchase-History.Generate",
        "purchaseReceiptGenerate":     "Purchase-Receipt.Generate",
        "purchaseIssuesResolve":       "Purchase-Issues.Resolve",
        "returnOrRefundInitiate":      "Return-Or-Refund.Initiate",
        "orderStatusTrack":            "Order-Status.Track",
        "budgetAfterPurchaseUpdate":   "Budget-After-Purchase.Update",
        "transactionCompleteVerify":   "Transaction-Complete.Verify",
        "purchaseInitiate":            "Purchase.Initiate",
        "purchaseEligibilityValidate": "Purchase-Eligibility.Validate",
...
_logger = get_logger("purchasing-agent-handler")
...
_agent = PurchasingAgent(capability_ctx=_capability_ctx)

_ROUTES: dict[str, str] = _load_pa_routes()
...
async def _execute_capability(
    ...
    if input_schema:
        try:
            ...
            _logger.write({
                "actor": "purchasing-agent-handler",
...
async def _dispatch(
    ...
    try:
        import importlib
        mod = importlib.import_module(f"agents.purchasing_agent.{module_name}")
        if hasattr(mod, "run"):
...
def lambda_handler(event, context):
    """
    Direct API Lambda entrypoint for the Purchasing-Agent (Mode B).

    Expected API Gateway HTTP API event:
      {
        "rawPath": "/agents/purchasingAgent/<capabilityPath>",
        "body": "{...}",
    ...
    Profile is per-request via capability_ctx.profile.
    For governed capabilities (Budget.Check with real vault data),
    use api_handler.py (Mode 3) instead.
    """
    try:
