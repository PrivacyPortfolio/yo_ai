...
def register_local_agent(agent_id: str, instance: Any) -> None:
    ...
    LOG.info("AgentRegistry.registered_local", message=f"Registered local agent '{agent_id}'.", payload={"agent_id": agent_id})
...
def register_lambda_agent(agent_id: str, function_name: str) -> None:
    ...
    LOG.info("AgentRegistry.registered_lambda", message=f"Registered Lambda agent '{agent_id}'.", payload={"agent_id": agent_id, "function_name": function_name})
...
class SolicitorGeneralAgent(PlatformAgent):
    ...
    def __init__(
        ...
        *,
        ...
        # Current request contexts — set at the start of each route/dispatch
        # call and available to self.log() throughout that call's execution.
        self._agent_ctx: Any = None
        self._capability_ctx: Any = None
    ...
    def log(
        ...
        *,
        ...
        """
        Write a platform log record using the current request contexts.

        self._agent_ctx and self._capability_ctx are set at the start of
        each route() / route_a2a() call so all log sites within that call
        automatically carry the full envelope without passing context objects
        ...
        LOG.write(
            ...
            capability_ctx=self._capability_ctx,
        )
    ...
    def route(
        ...
        self._capability_ctx = capability_ctx
    ...
    async def _dispatch_local(
        ...
        try:
            ...
            self.log(
                event_type="SG.RouteA2A.Local.Error",
                message=f"Local dispatch to '{target_id}' / '{capability_id}' failed — {exc}",
                payload={"targetAgentId": target_id, "capability": capability_id, "error": str(exc)},
                level="ERROR",
            )
            return self._error_envelope(
                correlation_id,
    ...
    async def _dispatch_lambda(
        ...
        try:
            ...
            self.log(
                event_type="SG.RouteA2A.Lambda.Error",
                message=f"Lambda dispatch to '{target_id}' / '{function_name}' failed — {exc}",
                payload={"targetAgentId": target_id, "functionName": function_name, "error": str(exc)},
                level="ERROR",
            )
            return self._error_envelope(
                correlation_id,
