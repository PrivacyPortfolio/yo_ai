# agents/complaint_manager/runtime/complaint_manager_handler.py
#
# AI-first Lambda handler for the Complaint-Manager agent.
#
# This handler is pure call_ai() — no run() modules yet.
# When run() modules are added, follow the dispatch pattern in
# door_keeper_handler.py (run() first, call_ai() fallback).
#
# Transport boundary responsibilities:
#   - Extract aws_request_id → seeds correlation_id
#   - Extract ctx_data from body → seeds all YoAiContext fields
#   - Build ctx before call_ai() so correlation is auditable
...
# slim=True: no fingerprints/knowledge/tools — appropriate for AI-first path.
# card and extended_card auto-loaded from agent_card/ bundle when passed.

_AGENT = ComplaintManagerAgent(slim=True)
...
def lambda_handler(event, context):
    ...
    try:
        ...
        if ctx.get("dry_run"):
            ...
            result = call_ai(
                {
                    "persona":    _AGENT.name,    # self.name from BaseAgent
