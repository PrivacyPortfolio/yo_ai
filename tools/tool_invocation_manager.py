...
class ToolInvocationManager:
    ...
    not by this manager. No credential material is handled here.
    """
    ...
    async def invoke(
        ...
        Tool boundary audit pattern:
          ...
          4. Log post-call entry — outcome, never raw output

        Raw tool output is never logged — it may contain personal data.

        Args:
            tool_name : Registered tool name (e.g. "AccessAdministrator")
            payload   : Tool input payload
            ctx       : YoAiContext — provides correlation, actor, dry_run
